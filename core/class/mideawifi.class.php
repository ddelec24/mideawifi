...
class mideawifi extends eqLogic {
	...
	/*
	* Récupération des infos des appareils*/
	public static function pull() {
      	//log::add('mideawifi', 'debug', 'Cron: Actualisation demandée.');
  ...
  public static function postConfig_cronMideawifi($value) {
    $listCron = ['cron1' => '* * * * *', 'cron5' => '*/5 * * * *', 'cron10' => '*/10 * * * *'];
    
    if(!array_key_exists($value, $listCron))
      	$value = 'cron5';
        ...
        if (!is_object($cron)) {
            ...
            $cron->setSchedule($listCron[$value]);
            $cron->setTimeout(2);
            $cron->save();
        } else {
            $cron->setEnable(1);
